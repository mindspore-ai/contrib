...
def exp_t(u, t):
    ...
    if t == 1.0:
        ...
        return ops.relu(1.0 + (1.0 - t) * u) ** (1.0 / (1.0 - t))
...
def compute_normalization_fixed_point(activations, t, num_iters=5):
    ...
    """
    activations = mindspore.Tensor(activations)
    mu = ops.max(activations, axis=-1)[0].view(-1, 1)
    ...
    while i < num_iters:
        i += 1
        logt_partition = ops.sum(exp_t(normalized_activations, t), dim=-1).view(-1, 1)
...
def tempered_softmax(activations, t, num_iters=5):
    ...
    if t == 1.0:
        normalization_constants = ops.log(ops.sum(ops.exp(activations), dim=-1))
    else:
...
def bi_tempered_logistic_loss(activations, labels, t1, t2, label_smoothing=0.0, num_iters=5):
    ...
    temp2 = (1 / (2 - t1)) * (ops.pow(labels, 2 - t1) - ops.pow(probabilities, 2 - t1))
    loss_values = temp1 - temp2

    return ops.sum(loss_values, dim=-1)
