...
if __name__ == "__main__":
    ...
    net = EmbedNet(pretrain="", low_dim=args.low_dim, class_num=n_class,\
               part=args.part, nheads=0, drop=0.2, alpha=0.2)
    ...
    if args.dataset == "SYSU":
        cmc, map_test, cmc_att, map_att = test(args, gallery_loader, query_loader, ngall,
                                               nquery, net, 1, gallery_cam=gall_cam, query_cam=query_cam)
    if args.dataset == "RegDB":
        if args.regdb_mode == "v2i":
            cmc, map_test, cmc_att, map_att = test(args, gallery_loader,
                                                   query_loader, ngall, nquery, net, 2)
        elif args.regdb_mode == "i2v":
            cmc, map_test, cmc_att, map_att = test(args, gallery_loader,
                                                   query_loader, ngall, nquery, net, 1)
    ...
    print('FC:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| map: {:.2%}'.format(
        cmc[0], cmc[4], cmc[9], cmc[19], map_test))
    print('FC:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| map: {:.2%}'.format(
        cmc[0], cmc[4], cmc[9], cmc[19], map_test), file=log_file)
