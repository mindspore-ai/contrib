...
class CriterionWithNet(nn.Cell):
    ...
    def construct(self, img10, img11, img2, label1, label2, adj, modal=0):
        """
        function of constructing
        """

        out_graph = None
        img1 = self.cat((img10, img11))

        if self._backbone.nheads > 0:
            feat, _, out, _, out_graph = self._backbone(
                img1, x2=img2, adj=adj, modal=modal)
        else:
            feat, _, out, _ = self._backbone(
                img1, x2=img2, modal=modal)

        label1 = self.cat((label1, label1))
...
class OptimizerWithNetAndCriterion(nn.Cell):
    ...
    def construct(self, x10, x11, x2, y1, y2, adj):
        """
        net grad and loss cal
        """
        loss = self.network(x10, x11, x2, y1, y2, adj)
        weights = self.weights
        # start_time = time.time()
        grads = self.grad(self.network, weights)(x10, x11, x2, y1, y2, adj)
        loss = P.Depend()(loss, self.optimizer(grads))
        # end_time = time.time()
        # print("Grad time using {:.2f}".format(end_time - start_time))
        return loss
