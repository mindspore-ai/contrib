...
class TrainOneStepCell(nn.Cell):
    ...
    def construct(self, x, target):
        ...
        loss = self.network(x, target)
        sens = ops.Fill()(ops.DType()(loss), ops.Shape()(loss), self.sens)
        grads = self.grad(self.network, weights)(x, target, sens)
        return F.depend(loss, self.optimizer(grads)), loss

def default_train(loader_train):
    """
    :param loader_train:
    :param loader_test:
    :return:
...
def train(loader_train, loader_test):
    ...
    for name in model_files:
        mox.file.rename(f'/home/work/user-job-dir/workspace/trained_models/{name}',
                        f'obs://test-ddag/output/AECRNet/trained_models/{name}')
    return psnrs, ssims


def save_list(saves, save_dir, save_name):
    """
    :param saves:
    :param save_dir:
    :param save_name:
    ...
    # else:
        ...
        os.mkdir(save_dir)
        print('dir created')
    file = open(save_dir + '/'+ save_name, 'w')
    for im in saves:
        file.write(str(im))
        file.write(str('\n'))
    file.close()
