...
class GCSemi:
    ...
    def __init__(self, train_relevance_labels, train_features,
        ...
        self.t1 = 100
        self.t2 = 300
        self.n_iter = 1000  # 2000
        # end of param ##

    def fit(self, fed_num, data_path, feature_num=16, from_fed=0, to_fed=4):
        """[summary]
        ...
        Args:
            ...
            to_fed (int, optional): [description]. Defaults to 4.
        """
        _ = data_path
        ...
        w1 = tf.Variable(tf.constant(ws1), name='w1')
        b1 = tf.Variable(tf.constant(bs1), name='b1')
        h1 = tf.add(tf.matmul(x, w1), b1)
        w2 = tf.Variable(tf.constant(ws2), name='w2')
        b2 = tf.Variable(tf.constant(bs2), name='b2')
        ...
        with tf.Session() as sess:
            ...
            for it in range(self.n_iter):
                ...
                _, loss = sess.run([opt, cost], feed_dict={
                    x: batch_x, y: batch_y, x_u: batch_x_unlabeled, pred_pl: batch_y_unlabeled, alpha: a})
                loss_one_fed.append(loss)
            draw([i for i in range(len(ndcg10_iters))], [ndcg10_iters])
            # save result
            self.save_result(sess, fed_num, w1, w2, b1, b2)
    def save_result(self, sess, fed_num, w1, w2, b1, b2):
        """[summary]
