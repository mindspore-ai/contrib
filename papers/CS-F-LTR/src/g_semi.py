...
class GSemi:
    ...
    def fit(self, from_fed, to_fed, _, feature_num=16):
        ...
        pred_us = tf.add(tf.matmul(x_u, ws), bs)
        ...
        with tf.Session() as sess:
            ...
            for it in range(self.n_iter):
                ...
                # batch_x_unlabeled += 0.2 * np.random.randn(batch_x_unlabeled.shape[0], batch_x_unlabeled.shape[1])
                batch_y_unlabeled = y_unlabeled[left: right]
                if self.is_change:
                    batch_y_unlabeled = (1 - self.r) * batch_y_unlabeled + self.r * sess.run(
                        tf.add(pred_u, self.beta), feed_dict={x_u: batch_x_unlabeled})
                else:
                    batch_y_unlabeled = (1 - self.r) * batch_y_unlabeled + self.r * \
                        sess.run(pred_us, feed_dict={x_u: batch_x_unlabeled})
            ...
            print("%f, %f, %f, %f;" %
                  (err_iters[-1], ndcg10_iters[-1], ndcg_iters[-1], map_iters[-1]))
