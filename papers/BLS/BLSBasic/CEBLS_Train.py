...
class CEBLSBasicTrain(N.Cell):
    def __init__(self) -> None:
        super(CEBLSBasicTrain, self).__init__()
        self.s = 0.8
        self.c = 2 ** -15
        self.N1 = 10
        self.N2 = 10
        ...
        self.stack_v_op = P.Stack(axis=1)
        self.stack_h_op = P.Stack(axis=0)
        self.diag_op = P.Diag()
        ...
        self.enhance_random_weight_list = Tensor(np.random.random((self.N3, self.N3, self.N3)),
                                                  dtype.float32)
    ...
    def construct(self, _train_data, _train_label):
        ...
        parameter_of_shrink = self.train(
    ...
    def train(self, x, y):
        ...
        weight_of_enhance_layer, cascade_weight_of_enhance_layer = self.generate_random_weight_of_enhance_layer()
        output_of_enhance_layer, parameter_of_shrink = \
            self.generate_output_of_enhance_layer(input_of_enhance_layer_with_bias, weight_of_enhance_layer, cascade_weight_of_enhance_layer)
        output, weight, concatenate_of_two_layer, pseudo_inverse = \
            self.generate_final_output(output_of_feature_mapping_layer, output_of_enhance_layer, y)
        return [[output, weight, weight_of_feature_layer, max_list_set, min_list_set, weight_of_enhance_layer, cascade_weight_of_enhance_layer, parameter_of_shrink],
    ...
    def generate_random_weight_of_enhance_layer(self):
        ...
        for i in range(self.N3):
            rand2 = self.enhance_random_weight_list[i]
            cascade_weight_of_enhance_layer.append(self.orthonormalize(2 * rand2 - self.fill_op(dtype.float32, rand2.shape, 1.0)))
        return self.squeeze_op(self.stack_v_op(weight_of_enhance_layer)), self.stack_v_op(cascade_weight_of_enhance_layer)
    ...
    def generate_final_output(self, _output_of_feature_mapping_layer, _output_of_enhance_layer, _train_label):
        ...
        pseudo_inverse = self.pinv(concatenate_of_two_layer, self.c)
        _output_weight = self.matmul_op(pseudo_inverse, _train_label)
        ...
        return _output_of_train, _output_weight, concatenate_of_two_layer, pseudo_inverse
    ...
    def generate_output_of_enhance_layer(self, _input_of_enhance_layer_with_bias, _weight_of_enhance_layer, _cascade_weight_of_enhance_layer_list):
        ...
        for i in range(self.N3):
            if i == 0:
                ...
                weight = _cascade_weight_of_enhance_layer_list[i]
...
if __name__ == '__main__':
    ...
    if not IS_EXPORT:
        ...
        while True:
            # print(">>>>>>>>>>>>>>>>>>>>>>")
            bls = CEBLSBasicTrain()
            parameter = bls.train(train_x, train_y)
            # print(">>>>>>>>>>>>>>>>>>>>>>")
            bls.accuracy_op.update(parameter[0][0], bls.argmax_op(train_y))
            print("BLS-Library Training Accuracy is : ", bls.accuracy_op.eval() * 100, " %")
            if bls.accuracy_op.eval() > 0.95:
                print("step", i)
                save_cebls_basic_ckpt(parameter, "../../checkpoints/ms_cebls_basic.ckpt",
                                    "../../checkpoints/ms_cebls_incre_prepare.ckpt")
                print("save sucess")
                break
            i+=1
    else:
