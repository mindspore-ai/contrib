...
class BLSBasicTest(N.Cell):
    def __init__(self) -> None:
        super(BLSBasicTest, self).__init__()
        self.s = 0.8
        self.c = 2 ** -15
        self.N1 = 10
        self.N2 = 10
    ...
    def generate_output_of_enhance_layer(self, input_of_enhance_layer_with_bias, weight_of_enhance_layer, parameter_of_shrink):
...
if __name__ == '__main__':
    ...
    data = bin_file.read()
    bin_file.close()
    train_data = Tensor(np.ndarray((86534, 78), float, buffer=data), dtype.float32)
    bin_file = open(bin_file_path_y, 'rb')
    data = bin_file.read()
    bin_file.close()
    train_label = Tensor(np.ndarray((86534, 2), float, buffer=data), dtype.float32)
    ...
    if not IS_EXPORT:
        ...
        output_of_test = bls.test(train_data, parameter)
        print(">>>>>>>>>>>>>>>>>>>>>>")
        bls.accuracy_op.update(output_of_test, bls.argmax_op(train_label))
        print("BLS-Library Testing Accuracy is : ", bls.accuracy_op.eval() * 100, " %")
    else:
        bls = BLSBasicTest()
        export(bls, train_data, train_label, file_name="testbls.air", file_format='AIR')