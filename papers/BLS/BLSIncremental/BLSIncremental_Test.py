...
import numpy as np
import copy
from utils.BLS_load_checkpoint import load_bls_basic_ckpt
from utils.BLS_load_checkpoint import load_bls_incremental_ckpt


class BLSIncrementalTest(N.Cell):
    def __init__(self) -> None:
        super(BLSIncrementalTest, self).__init__()
        self.s = 0.8
        self.c = 2 ** -15
        self.N1 = 10
        self.N2 = 10
    ...
    def test(self, test_data, parameter, return_parameter):
        ...
        [[output_of_train, output_weight],
         [weights_of_feature_layer, max_list_set, min_list_set],
...
if __name__ == '__main__':
    ...
    parameter = load_bls_basic_ckpt('../../data/param2.ckpt')
    basic_data = train_data[0: (int)(inputData), :]

    if not IS_EXPORT:
        # print(">>>>>>>>>>>>>>>>>>>>>>")
        bls = BLSIncrementalTest()
        for e in range(0, l - 1):
            return_parameter = load_bls_incremental_ckpt("../../data/param_"+str(e+1)+".ckpt")
