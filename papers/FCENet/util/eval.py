...
def deal_eval_total_text(debug=False):
    ...
    if debug:
        ...
        pth_name = cfg.resume.split('/')[-1].replace('.pth', '')

        if not os.path.exists(outpt_dir_base):
            mkdirs(outpt_dir_base)

        outpt_dir1 = os.path.join(outpt_dir_base, "[{}]_[{},{}]_{}_{}_{}_{}"
                                  .format(pth_name, cfg.test_size[0], cfg.test_size[1], cfg.tr_thresh,
                                          cfg.nms_thresh, 0.7, 0.6))
        osmkdir(outpt_dir1)
        fid_path1 = '{}/Eval_TotalText_{}_{}.txt'.format(eval_dir, 0.7, 0.6)

        analysize_result(source_dir, fid_path1, outpt_dir1, "totalText")

        outpt_dir2 = os.path.join(outpt_dir_base, "[{}]_[{},{}]_{}_{}_{}_{}"
                                  .format(pth_name, cfg.test_size[0], cfg.test_size[1], cfg.tr_thresh,
                                          cfg.nms_thresh, 0.8, 0.4))
        osmkdir(outpt_dir2)
...
def deal_eval_ctw1500(debug=False):
    ...
    if debug:
        ...
        pth_name = cfg.resume.split('/')[-1].replace('.ckpt','')
        outpt_dir = os.path.join(outpt_dir_base, "[{}]_[{},{}]_{}_{}".format(pth_name, cfg.test_size[0],
                                                                             cfg.test_size[1], cfg.tr_thresh,
                                                                             cfg.nms_thresh))
        osmkdir(outpt_dir)
