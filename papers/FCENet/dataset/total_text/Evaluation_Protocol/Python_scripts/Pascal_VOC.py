...
input_dir = 'output/Totaltext' #detection directory goes here
gt_dir = 'data/total-text-mat/gt/Test' #gt directory goes here
fid_path = 'dataset/total_text/result.txt' #output text file directory goes here
    ...
    if (input_id != '.DS_Store'):
        ...
        for gt_id, gt in enumerate(groundtruths):
            if len(detections) > 0:
                for det_id, detection in enumerate(detections):
                    detection = detection.split(',')
                    detection = list(map(int, detection)) #take it off in the final script
                    det_y = detection[0::2]
                    det_x = detection[1::2]
                    gt_x = list(map(int, np.squeeze(gt[1])))
                    gt_y = list(map(int, np.squeeze(gt[3])))
