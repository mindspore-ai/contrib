...
class MetaDataset:
    ...
    def get_img_mask(self, img, box, merge=True):
        ...
        if cfg.metain_type in [3, 4]:
            ...
            img = mindspore.ops.Concat([img, croped])
        else:
            img = self.meta_transform(img)
        ...
        if x1 == x2 or y1 == y2:
            ...
            mask = mindspore.ops.Zeros((1, h, w))
            mask[:, y1:y2, x1:x2] = 1

        if merge:
            return mindspore.ops.Concat([img, mask]) 
        else:
            return img, mask
...
class MetaDataset_without_data_argumentation:
    def __init__(self,
            metafiles,
            transform=None,
            target_transform=None,
...
if __name__ == '__main__':
    ...
    metaloader = MetaDataset(metaset, column_names=['img']).batch(BATCH_SIZE)
    batch_size = 64
