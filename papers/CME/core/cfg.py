...
def load_conv(buf, start, conv_model):
    ...
    if conv_model.bias is not None:
        ...
        conv_model.bias.data.copy_(Tensor.from_numpy(buf[start:start+num_b]));   start = start + num_b
    #conv_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w]))
    #conv_model.weight.data.copy_(Tensor.from_numpy(buf[start:start + num_w]).view(conv_model.weight.data.shape[0],conv_model.weight.data.shape[1]))
    #print(conv_model.weight.data.shape)
    conv_model.weight.data.copy_(Tensor.from_numpy(buf[start:start + num_w]).view(conv_model.weight.data.shape))
    start = start + num_w
    return start

def load_convfromcoco(buf, start, conv_model):
    print('------ loading coco to voc ----------')
    tmpb = mindspore.ops.Zeros(425)
    tmpw = mindspore.ops.Zeros(425, 1024, 1, 1)
    num_w = tmpw.numel()
    inds = np.concatenate([
    ...
    if conv_model.bias is not None:
        num_b = tmpb.numel()
        tmpb.copy_(Tensor.from_numpy(buf[start:start+num_b])); start = start + num_b
        conv_model.bias.data.copy_(tmpb[allinds]);   
    tmpw.copy_(Tensor.from_numpy(buf[start:start+num_w])); start = start + num_w
...
def load_conv_bn(buf, start, conv_model, bn_model):
    ...
    bn_model.bias.data.copy_(Tensor.from_numpy(buf[start:start+num_b]));     start = start + num_b
    bn_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_b]));   start = start + num_b
    bn_model.running_mean.copy_(Tensor.from_numpy(buf[start:start+num_b]));  start = start + num_b
    bn_model.running_var.copy_(Tensor.from_numpy(buf[start:start+num_b]));   start = start + num_b
    #conv_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w])); start = start + num_w 
    conv_model.weight.data.copy_(Tensor.from_numpy(buf[start:start + num_w]).view(conv_model.weight.data.shape))
    start = start + num_w
    return start
...
def load_fc(buf, start, fc_model):
    if fc_model.bias is None:
        ...
        print(num_w)
        #print()
        #fc_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w]).resize(num_b,num_w//num_b));   start = start + num_w
        fc_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w]).view(fc_model.weight.shape));   start = start + num_w
    else:
        num_w = fc_model.weight.numel()
        ...
        fc_model.bias.data.copy_(Tensor.from_numpy(buf[start:start+num_b]));     start = start + num_b
        print(fc_model.weight.shape)
        #fc_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w]).resize(fc_model.weight.shape));   start = start + num_w
        fc_model.weight.data.copy_(Tensor.from_numpy(buf[start:start+num_w]).view(fc_model.weight.shape));   start = start + num_w
    return start
