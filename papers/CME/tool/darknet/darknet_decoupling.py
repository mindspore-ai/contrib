...
def maybe_repeat(x1, x2):
    ...
    if n1 == n2:
        ...
        shape = x1.shape[1:]
        nc = n2 // n1
        x1 = mindspore.repeat(x1,(nc, *[1]*x1.ndim()))
        x1 = mindspore.numpy.transpose(x1,(0,1))
        x1 = x1.view(-1, *shape)
    else:
        assert n1 % n2 == 0
        shape = x2.shape[1:]
        nc = n1 // n2
        x2 = mindspore.repeat(x2,(nc, *[1]*x2.ndim()))
        x2 = mindspore.numpy.transpose(x2,(0,1))
        x2 = x2.view(-1, *shape)
    return x1, x2
...
class MaxPoolStride1(nn.Cell):
    def __init__(self):
        super(MaxPoolStride1, self).__init__()

    def forward(self, x):
        x = nn.MaxPool2d(nn.pad(x, (0,1,0,1), mode='replicate'), 2, stride=1)
        return x
...
class Reorg(nn.Cell):
    ...
    def forward(self, x):
        ...
        ws = stride
        hs = stride
        x = mindspore.numpy.transpose(x.view(B, C, H//hs, hs, W//ws, ws),(3,4))
        x = mindspore.numpy.transpose(x.view(B, C, H//hs*W//ws, hs*ws),(2,3))
        x = mindspore.numpy.transpose(x.view(B, C, hs*ws, H//hs, W//ws),(1,2))
        x = x.view(B, hs*ws*C, H//hs, W//ws)
        return x


# for route and shortcut
class EmptyModule(nn.Cell):
    def __init__(self):
...
class Darknet(nn.Cell):
    def __init__(self, darknet_file, learnet_file):
        ...
        self.header = Tensor(([0,0,0,0]),dtype=mindspore.int16)
        self.seen = 0
    ...
    def meta_forward(self, metax, mask):
        ...
        for i in range(int(self.learnet_blocks[0]['feat_layer'])):
            if i == 0 and metax.size(1) == 6:
                done_split = True
                metax = ops.concat(ops.split(metax, 3, dim=1))
            metax = self.models[i](metax)
        if done_split:
            metax = ops.concat(ops.split(metax, int(metax.size(0)/2)), dim=1)
        if cfg.metain_type in [2, 3]:
            metax = ops.concat([metax, mask], dim=1)
        ...
        for learnet_block in self.learnet_blocks:
            ...
            if learnet_block['type'] == 'learnet':
                ...
                metax = metax.view(metax.shape[0], metax.shape[1])
                metax = self.learnet_models[ind](metax)
                outputs[ind] = metax
                metax = ops.ExpandDims(metax,2)
                metax = ops.ExpandDims(metax,3)
    ...
    def construct(self, x, metax, mask, ids=None):
        # pdb.set_trace()
    ...
    def create_network(self, blocks):
        ...
        for block in blocks:
            if block['type'] == 'net' or block['type'] == 'learnet':
                ...
                if activation == 'leaky':
                    model.add_module('leaky{0}'.format(conv_id), nn.LeakyReLU(0.1))
                elif activation == 'relu':
                ...
                if block['activation'] == 'linear':
                    model = nn.Dense(prev_filters, filters)
                elif block['activation'] == 'leaky':
                    model = nn.SequentialCell(
                               nn.Dense(prev_filters, filters),
    ...
    def load_weights(self, weightfile):
        ...
        self.header = Tensor.from_numpy(header)
