...
def weights_init_he(m):
    if isinstance(m, nn.Conv2d):
        if hasattr(m, 'weight'):
            w1 = Parameter(initializer(HeUniform(), list(m.weight.shape), mindspore.float32))
            m.weight.set_data(w1)
            w2 = Parameter(initializer(Zero(), list(m.bias.shape), mindspore.float32))
            m.bias.set_data(w2)
        else:
            print("error")
...
class DUnet(nn.Cell):
    def __init__(self, in_channels, weights_init=True):
        ...
        self.Expand = Expand
        self.MaxPool3d = nn.MaxPool3d(kernel_size=2, stride=2, pad_mode='pad', padding=0, dilation=1, return_indices=False)
        self.MaxPool2d = nn.MaxPool2d(kernel_size=2, stride=2, pad_mode='pad', padding=0, dilation=1, return_indices=False)
        ...
        self.conv_10 = nn.SequentialCell([
            nn.Conv2d(in_channels * 8, 1, kernel_size=1, padding=0, has_bias=True),
            nn.Sigmoid()
        ])
...
if __name__ == "__main__":
    ...
    BATCH_SIZE = 1
    input_batch = mindspore.Tensor(BATCH_SIZE, 4, 192, 192)
    output_batch = model(input_batch)

    print(output_batch.shape)  # BATCH_SIZE, 1, 192, 192
