...
"""
MindSpore implementation of `Swin Transformer`.
Refer to "Swin Transformer: Hierarchical Vision Transformer using Shifted Windows"
"""
import mindspore as ms
from mindspore import nn
from mindspore.common.initializer import initializer, TruncatedNormal

from swin_transformer.helper import build_model_with_cfg
from swin_transformer.layers import DropPath, to_2tuple
from swin_transformer.registry import register_model

IMAGENET_DEFAULT_MEAN = (0.485, 0.456, 0.406)
IMAGENET_DEFAULT_STD = (0.229, 0.224, 0.225)

__all__ = ['SwinTransformer', 'swin_base_patch4_window7_224']
...
class WindowAttention(nn.Cell):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = ms.Parameter(
            ms.ops.zeros(((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads)))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        self.relative_position_index = ms.Parameter(relative_coords.sum(-1))  # Wh*Ww, Wh*Ww
...
class PatchMerging(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = ms.ops.cat([x0, x1, x2, x3], axis=-1)  # B H/2 W/2 4*C
...
class BasicLayer(nn.Cell):
    ...
    Args:
        ...
        fused_window_process (bool, optional): If True, use one kernel to fused window shift & window partition
...
def _create_swin_transformer(variant, pretrained=False, **kwargs):
    ...
    return build_model_with_cfg(SwinTransformer, pretrained, variant, **kwargs)
...
def swin_base_patch4_window12_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"img_size": 384, "patch_size": 4, "window_size": (12, 12, 12, 12), "embed_dim": 128,
                    "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32), **kwargs}
...
def swin_base_patch4_window7_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"patch_size": 4, "window_size": (7, 7, 7, 7), "embed_dim": 128,
                    "depths": (2, 2, 18, 2), "num_heads": (4, 8, 16, 32), **kwargs}
...
def swin_large_patch4_window12_384(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"img_size": 384, "patch_size": 4, "window_size": (12, 12, 12, 12), "embed_dim": 192,
                    "depths": (2, 2, 18, 2), "num_heads": (6, 12, 24, 48), **kwargs}
...
def swin_large_patch4_window7_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"patch_size": 4, "window_size": (7, 7, 7, 7), "embed_dim": 192,
                    "depths": (2, 2, 18, 2), "num_heads": (6, 12, 24, 48), **kwargs}
...
def swin_small_patch4_window7_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"patch_size": 4, "window_size": (7, 7, 7, 7), "embed_dim": 96,
...
def swin_large_patch4_window7_224_in22k(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"patch_size": 4, "window_size": (7, 7, 7, 7), "embed_dim": 192, "depths": (2, 2, 18, 2),
...
def swin_s3_tiny_224(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = {"patch_size": 4, "window_size": (14, 14, 14, 7), "embed_dim": 96, "depths": (2, 2, 18, 2),
