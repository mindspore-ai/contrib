...
class USAM(Optimizer):
    def __init__(self, base_optimizer: Optimizer, rho=0.05, adaptive=False, stable=False):
        ...
        super(USAM, self).__init__(base_optimizer.parameters, self.defaults)
    ...
    def first_step(self, gradients):
        ...
        for group_id, group in enumerate(self.param_groups):
            ...
            for i, p in enumerate(group['params']):
                ...
                e_w = (ops.pow(p, 2) if group["adaptive"] else 1.0) * grad * group["rho"]
...
if __name__ == '__main__':
    ...
    usam_optimizer = USAM(base_optimizer)
    grad_fn = ms.value_and_grad(forward_fn, None, usam_optimizer.parameters, has_aux=True)
    train(model, train_loader, usam_optimizer, grad_fn)
