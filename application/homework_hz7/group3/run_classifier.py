...
from src.utils import make_directory, LossCallBack, load_newest_ckpt, BertLearningRate

_cur_dir = os.getcwd()


def do_train(dataset=None, network=None, load_checkpoint_path_bert="", load_checkpoint_path_vit="",
             save_checkpoint_path="", epoch_num=1):
    """ do train """
    for i in dataset:
        print(np.shape(i[2]))
        break
    if load_checkpoint_path_bert == "" or load_checkpoint_path_vit == "":
    ...
    if optimizer_cfg.optimizer == 'AdamWeightDecay':
        lr_schedule = BertLearningRate(learning_rate=optimizer_cfg.AdamWeightDecay.learning_rate,
                                       ...
                                       warmup_steps=int(steps_per_epoch * epoch_num * 0.01),
    ...
    ckpoint_cb = ModelCheckpoint(prefix="classifier",
                                 ...
                                 config=ckpt_config)
    param_dict = load_checkpoint(load_checkpoint_path_bert)
    load_param_into_net(network, param_dict, strict_load=False)
    # param_dict = load_checkpoint(load_checkpoint_path_vit)
    # load_param_into_net(network, param_dict, strict_load=False)
...
def do_eval(dataset=None, network=None, num_class=2, load_checkpoint_path=""):
    ...
    columns_list = ["input_ids", "input_mask", "frame", "frame_mask", "label_ids"]
    df = []
    for data in tqdm(dataset.create_dict_iterator(num_epochs=1)):
        input_data = []
        for i in columns_list:
            input_data.append(data[i])
        input_ids, input_mask, frame, frame_mask, label_ids = input_data
        logits = model.predict(input_ids, input_mask, frame, frame_mask, label_ids)
        df.append(logits)
...
def modelarts_pre_process():
    ...
    args_opt.load_pretrain_checkpoint_path = os.path.join(file_dir, args_opt.load_pretrain_checkpoint_path)
...
def run_classifier():
    ...
    load_pretrain_checkpoint_path_bert = args_opt.load_pretrain_checkpoint_path_bert
    load_pretrain_checkpoint_path_vit = args_opt.load_pretrain_checkpoint_path_vit
    ...
    netwithloss = BertCLS(bert_net_cfg, True, num_labels=args_opt.num_class, dropout_prob=0.1,
                          assessment_method=assessment_method, bs=args_opt.train_batch_size)
    ...
    if args_opt.do_train.lower() == "true":
        ...
        do_train(ds, netwithloss, load_pretrain_checkpoint_path_bert, load_pretrain_checkpoint_path_vit,
    ...
    if args_opt.do_eval.lower() == "true":
        ...
        do_eval(ds, BertCLS, args_opt.num_class, load_finetune_checkpoint_path)
